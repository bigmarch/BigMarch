...
public class MeshFan : MonoBehaviour
{
	...
	private class SectorMeshCreator
	{
		...
		/// </param>
		/// <returns></returns>
		public Mesh CreateMesh(float radius, float angleDegree, int segments, int angleDegreePrecision,
			int radiusPrecision)
		{
		...
		private Mesh Create(float radius, float angleDegree, int segments)
		{
			...
			for (int i = 1; i < vertices.Length; i++)
			{
				...
				uvs[i] = new Vector2(Mathf.Sin(currAngle) * .5f + .5f, Mathf.Cos(currAngle) * .5f + .5f);
		...
		private bool checkDiff(float radius, float angleDegree, int segments, int angleDegreePrecision,
			int radiusPrecision)
		{
